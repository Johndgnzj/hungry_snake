...
var MovingGroundSinkUnit = 50; // 每下沈單位,越低越慢
...
function SetFallingSky(level) {
	$(".fallingsky").css('top', (-$(".fallingsky img").height() + DropitemSize * (level - 1)) + 'px');
}
// 設定物件可拖移
function DragSetup() {
	$("#snake").draggable({
...
function GameOver() {
	...
	showMsg('gameover');
	// LOCAL 執行 跳過
	document.location.href = 'hungrysnake.html?guid=1234';
	// $.ajax({
	// 	method: "POST"
	// 	, processData: false
